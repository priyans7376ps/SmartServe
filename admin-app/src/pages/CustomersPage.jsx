          ...
          <div className="space-y-6 text-xs">
            <div className="flex items-center gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="w-12 h-12 rounded-full bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-500 font-extrabold text-lg">
                {(selectedCustomer.full_name || 'G').charAt(0)}
              </div>
              <div>
                <h4 className="text-base font-bold text-slate-900 dark:text-white">{selectedCustomer.full_name || 'Guest Customer'}</h4>
                <p className="text-slate-400">{selectedCustomer.email || 'N/A'}</p>
                <p className="text-slate-400">{selectedCustomer.phone || 'N/A'}</p>
              </div>
            </div>
            ...
            <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/60 dark:border-slate-800">
              ...
              <p className="text-lg font-extrabold text-slate-900 dark:text-white">{selectedCustomer.loyalty_points || 0} pts</p>
            </div>

            <div className="space-y-2">
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-400 font-medium">Total Orders Placed:</span>
                <span className="font-bold text-slate-900 dark:text-white">{selectedCustomer.total_orders}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-400 font-medium">Total Spending:</span>
                <span className="font-bold text-amber-500">₹{Number(selectedCustomer.total_spending || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-400 font-medium">Average Order Value:</span>
                <span className="font-bold text-slate-900 dark:text-white">₹{Number(selectedCustomer.avg_order_value || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-400 font-medium">Member Since:</span>
                <span className="font-bold text-slate-900 dark:text-white">
                  {selectedCustomer.registration_date ? new Date(selectedCustomer.registration_date).toLocaleDateString() : 'N/A'}
                </span>
              </div>
            </div>
          </div>
